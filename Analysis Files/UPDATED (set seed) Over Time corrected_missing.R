...
train <- data_preproess(train)
test <- data_preproess(test)
...
model_region_source <- glm(rep_formula, data=source, family=binomial)
...
write.csv(internal_val_results_df, "internal_validation-over_time-corrected_missing.csv")



boot_results<- manual_boot(source, target, 200, rep_formula)

write.csv(boot_results$apparent, "bootstrap_apparent_validation_results-over_time-corrected_missing.csv")
write.csv(boot_results$test, "bootstrap_test_validation_results-over_time-corrected_missing.csv")
