...
print("source sample size < 2010")
print(nrow(source))

print("target sample size >= 2010")
print(nrow(target))

print("validation 2012-2014 sample size")
print(nrow(validation_region))
...
after2010_val_train <- validation_results(model_region_source, target)

after2010_val_external <- validation_results(model_region_source, validation_region)
...
external_prop_Lim_val <- ps_validation_results(propensity_model_limit1, validation_region) #validated on target from kiddle validation data (more recent 2012-2014)
...
external_prop_lambda_val <- ps_validation_results(propensity_model_lambda, validation_region)
...
external_int_full_val <- calibrated_model_validation_results(int_calib_full_df, validation_region, "intercept_only")
...
extrenal_int_source_val <- calibrated_model_validation_results(int_calib_source_only, validation_region, "intercept_only")
...
external_logistic_full_val <- calibrated_model_validation_results(logit_calib_full_df, validation_region, "logistic")
...
external_logistic_source_val <- calibrated_model_validation_results(logit_calib_source_only, validation_region, "logistic")
...
external_model_on_allData_val <- ps_validation_results(model_on_allData, validation_region)
...
external_model_on_targetOnly_val <- validation_results(model_on_targetOnly, validation_region)
...
internal_val_results_df <- rbind(
  after2010_val_train$val_results,
...
write.csv(internal_val_results_df, "internal_validation-before2010_after2010_corrected_missing.csv")

external_val_results_df <- rbind(
...
external_val_results_df$model_name <- Models

write.csv(external_val_results_df, "external_validation-before2010_after2010_corrected_missing.csv")



boot_results<- manual_boot(source, target, 200, rep_formula)

write.csv(boot_results$apparent, "bootstrap_apparent_validation_results-before2010_after2010_corrected_missing.csv")
write.csv(boot_results$test, "bootstrap_test_validation_results-before2010_after2010_corrected_missing.csv")
